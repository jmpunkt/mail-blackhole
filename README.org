...
Sometimes it is necessary to run an application on a path-prefix
(e.g. /app). To enable this behavior for this application, the
environment flag `BASE_URL` must be set for both builds. It is not
possible to alter this behavior with configuration. If your prefix
changes you have to re-compile the application.

Together with Nginx and the following configuration, multiple
applications can run within the same domain, separated by prefixes.
